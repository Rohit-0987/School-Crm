...
function Table({ modelName }) {
  ...
  const selectedFieldsMap = {
    ...
    class:['name','year','assignedTeacher.name','maxCapacity','_id']
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const rowsWithSelectedFields = data.map((row, index) => {
        ...
        if (lowerCaseModelName === 'teacher' && row.assignedClass) {
          selectedValues['assignedClass.name'] = row.assignedClass.name;
        } else if (lowerCaseModelName === 'class') {
          
          if (typeof row.assignedTeacher === 'string') {
            selectedValues['assignedTeacher.name'] = row.assignedTeacher;
          } else if (row.assignedTeacher && row.assignedTeacher.name) {
            selectedValues['assignedTeacher.name'] = row.assignedTeacher.name;
          }
        }
        return {
          id: index + 1,
